            ...
            if self.debug_save_gifs:
                ...
                if time_since_last >= self.save_interval and len(self.frame_buffer) > 0:
                    self.get_logger().info(
                        f"Attempting to save GIF. Buffer size: {len(self.frame_buffer)}"
                    )
                    timestamp = time.strftime("%Y%m%d-%H%M%S")
                    filename = os.path.join(
                        self.save_dir, f"camera_sequence_{timestamp}.gif"
                    )
                    ...
                    try:
                        ...
                        imageio.mimsave(
                            filename, self.frame_buffer, duration=0.033, loop=0
                        )
                        ...
                        self.get_logger().error(
                            f"Save directory exists: {os.path.exists(self.save_dir)}"
                        )
                        self.get_logger().error(
                            f"Save directory permissions: {oct(os.stat(self.save_dir).st_mode)[-3:]}"
                        )
