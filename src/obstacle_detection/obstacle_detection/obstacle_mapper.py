...
class ObstacleMapper(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameters(
            ...
            [
                ...
                ("debug_save_maps_interval", 10.0),  # how often to save maps, in seconds
                ("debug_save_maps_max_maps", 10),  # max number of maps to have saved at a time
                ...
                ("default_map_resolution", 0.1),  # Resolution of the default map in meters per cell
            ],
        )
    ...
    def init_default_map(self) -> None:
        ...
        if self.map is None:
            ...
            self.get_logger().info(f"Initialized default map from config: {width}x{height}, resolution: {resolution}")
    ...
    def compass_callback(self, msg: Float64) -> None:
        ...
        if self.last_compass_time is not None:
            time_diff = (current_time.nanoseconds - self.last_compass_time.nanoseconds) / 1e9  # To seconds
        else:
            time_diff = 0.0
    ...
    def gps_callback(self, msg: GPS) -> None:
        ...
        self.get_logger().debug(f"Current position: {self.current_latitude}, {self.current_longitude}")
    ...
    def map_callback(self, msg: OccupancyGrid) -> None:
        ...
        if msg.info.width <= 0 or msg.info.height <= 0:
            self.get_logger().warn(f"Received map with invalid dimensions: {msg.info.width}x{msg.info.height}")
            return
        ...
        map_changed = (self.map is not None and 
                    (self.map_width != msg.info.width or 
                    self.map_height != msg.info.height or 
                    self.map_resolution != msg.info.resolution))
        
        # Only update the entire map data if this is the first map or if dimensions changed
        if is_first_map or map_changed:
            self.get_logger().info("Received new map or map dimensions changed, updating map data")
            self.map = msg
            ...
            self.get_logger().debug("Received map update, keeping obstacle data and updating metadata only")
    ...
    def detection_callback(self, msg: Float32MultiArray) -> None:
        ...
        if has_spatial_data:
            ...
            self.get_logger().info(f"Detection: angle={angle:.1f}°, height={height:.1f}mm, distance={distance:.1f}mm")
        ...
        if not self.compass_received:
            self.get_logger().warn("No compass data received yet, assuming heading of 0 degrees (North)")
        ...
        self.get_logger().info(f"Using heading: {current_heading}° for detection processing")
        ...
        self.get_logger().debug(f"Robot heading: {current_heading}°, Camera angle: {angle}°, Global angle: {global_angle}°")
        ...
        if has_spatial_data:
            ...
            dx = robot_frame_x * math.cos(heading_rad) - robot_frame_y * math.sin(heading_rad)
            dy = robot_frame_x * math.sin(heading_rad) + robot_frame_y * math.cos(heading_rad)
        ...
        if grid_x < 0 or grid_x >= self.map_width or grid_y < 0 or grid_y >= self.map_height:
            self.get_logger().warn(f"Obstacle coordinates ({grid_x}, {grid_y}) outside map bounds")
            return
    ...
    def update_map_with_obstacle(self, grid_x: int, grid_y: int, obstacle_radius: int) -> None:
                    ...
                    if 0 <= x < self.map_width and 0 <= y < self.map_height:
                        ...
                        if 0 <= index < len(map_data):
                            ...
                            if distance_ratio < 0.3:  # Core of the obstacle
                                ...
                                occupancy = int(self.OBSTACLE_VALUE * math.exp(-2 * distance_ratio))
                            ...
                            if current_value < 0:  # Unknown space
                                ...
                                map_data[index] = occupancy
                                cells_updated += 1
                            elif current_value < 100 and occupancy > 0:  # Reinforce existing obstacle
                                # Weighted average with more weight to higher value
                                map_data[index] = min(100, int(current_value * 0.7 + occupancy * 0.3))
                                cells_updated += 1
        ...
        # Publish updated map
        self.map_publisher.publish(updated_map)
        self.get_logger().info(f"Map updated with obstacle at ({grid_x}, {grid_y}), {cells_updated} cells updated")
    ...
    def get_robot_map_position(self) -> Tuple[float, float]:
        ...
        if self.current_latitude is not None and self.current_longitude is not None:
            ...
            self.get_logger().debug("GPS to map conversion not implemented, using map center")
            ...
            self.get_logger().debug(f"Map center in grid coordinates: ({grid_center_x}, {grid_center_y})")
            ...
            grid_center_y = int((center_y - self.map_origin_y) / self.map_resolution)
            self.get_logger().debug(f"Map center in grid coordinates: ({grid_center_x}, {grid_center_y})")
            self.get_logger().debug(f"No GPS data, assuming robot at map center ({center_x:.2f}, {center_y:.2f})")
    ...
    def check_save_map(self) -> None:
        ...
        if time_since_last >= self.save_interval:
            self.get_logger().info(f"Time to save map image (interval: {self.save_interval}s)")
            self.save_map_image()
        ...
        try:
            ...
            cmap = colors.LinearSegmentedColormap.from_list(
                ...
                [(0.7, 0.7, 0.7),    # Light gray for unknown (-1)
                 ...
                 (1.0, 0.0, 0.0)],   # Red for high occupancy
                N=102
            )
            ...
            if self.current_heading is not None:
                ...
                plt.arrow(grid_x, grid_y, heading_dx, heading_dy,
            ...
            if self.recent_detections:
                ...
                for i, detection in enumerate(self.recent_detections):
                    ...
                    if i == len(self.recent_detections) - 1 and 'radius' in detection:
                        circle = plt.Circle((det_grid_x, det_grid_y), detection['radius'],
                                          fill=False, color='red', alpha=0.7)
